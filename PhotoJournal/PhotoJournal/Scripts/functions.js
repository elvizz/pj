...
(function($){
	$.fn.extend({
		...
		kotodama_TwitterWidget		: function(){
		...
		// --- -- - BACK TO TOP PLUGIN - -- --- //
		kotodama_BackToTop	: function( content ){
		
			var el	= $('<a />', {
				id		: "backToTop",
				href	: "#",
				text	: content || "Back to top"
			}).appendTo("body");
			
			function init(){
				var windowWidth	= $("body").width();
				if( windowWidth > 960 ){
					posLeft	= ( ( windowWidth - 960 )/2 ) + 960;				
					el.animate({ left : posLeft }, 100);
				}else{
					el.css({ left : windowWidth+50 });
				}
			}
			...
			// Show it when you scroll 200 pixels down
			...
			// Recalculate the position when resizing the window
			$(window).resize(function(){
				init();
			});
			
			// Show it to preload the image
...
jQuery(document).ready(function($){
	...
	mtdScript = {
		...
		initSite	: function(){
			...
			$(".twitterWidget").kotodama_TwitterWidget();
			...
			// Back to top button
			$.fn.kotodama_BackToTop("Back to top");
			
			// Initialize contact form
			$(".contactForm").kotodama_ContactForm();
			
			// Load the Google Map using latitude and longitude values
			$(".googleMap").kotodama_GoogleMap();
			
			// Hover effect on compact portfolio items
			$(".innerDesc").each(function(){
			
				var thumbHolder	= $(this).find(".thumbHolder"),
					thumb		= thumbHolder.find(".thumb"),
					opacity		= 0.2;
					
				thumbHolder.hover(function(){
					thumb.stop(true,true).fadeTo(200, opacity);
				},function(){
					thumb.stop(true,true).fadeTo(200, 1);
				});
				
			});
			
			// Hover effect on lightbox thumbnails
